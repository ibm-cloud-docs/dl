...
1. Specify these IBM Cloud details:
   ...
   * **Provider IP Address** (optional) – The IP address space in CIDR format assigned in the IBM network for peering. This field is optional and if left blank, Megaport automatically assigns a private `/30` address. If specifying a range, this must be the other useable IP in the range specified for the customer IP.
