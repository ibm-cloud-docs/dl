...
Direct Link on Classic Deprecation Update
:    Following the end-of-marketing date on 15 September 2025, Direct Link on Classic (1.0) through Equinix is no longer available for new deployments, as new orders are no longer accepted. Atlanta (ATL) remains the last location where Direct Link on Classic is still operational.
...
:    MACsec, a security protocol that provides encryption for data over Ethernet, is now available in select locations equipped with cross-connect switches (XCS) that support MACsec functionality. Supported markets include Chennai, Dallas, Madrid, Montreal, Toronto, and Mumbai. For more information, see [Planning for the Direct Link Dedicated MACsec feature](/docs/dl?topic=dl-dl-planning-considerations#macsec-feature-dedicated).
