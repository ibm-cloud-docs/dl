...
| Management | {{site.data.keyword.IBM_notm}} provisions and manages the {{site.data.keyword.dl_short}} gateway. | The Customer is responsible for:<ul><li>Following instructions in the {{site.data.keyword.dl_full_notm}} user documentation to make any necessary configurations, such as BGP.</li><li>Setting up routing, adding virtual connections to resources, and adjusting networking configurations to meet the needs of their workloads.</li><li>Making sure that any self-provided BGP addresses do not conflict with blocks that are used by IBM, or by resources external to your deployment.</li><li>Ensuring that there are no IP address conflicts between on-premises subnets, and subnets on IBM Cloud for both VPC and classic infrastructure connections.</li></ul> |
